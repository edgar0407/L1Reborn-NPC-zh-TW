...
- 新增翻譯：`eldnas2-e.html`/`.html`（艾爾納斯，火龍聖域入口，60級以上獻上「極寒之氣」進入）、
  `fd_death-e.html`/`.html`（墮落騎士，需擊敗三隻火龍利歐才能過橋進入聖域）——巴拉卡斯（Valakas）
  官方既有譯名，其餘「火焰谷」「極寒之氣」「火龍利歐」「聖域」為本次新譯
...
- 已比對 `RefrenceOnly/Reborn-20260831` 全量 155 個檔案，其餘 147 個與過去快照逐位元組相同，資料夾已清成只留這次真正的 delta
